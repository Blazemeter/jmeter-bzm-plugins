...
public class HlsSampler extends AbstractSampler {
    ...
    private DataRequest getMasterList(SampleResult masterResult, Parser parser) throws IOException {
        ...
        DataRequest respond = parser.getBaseUrl(getURLData(), masterResult, true);
        ...
        int headerBytes =
                masterResult.getResponseHeaders().length()   // condensed length (without \r)
        ...
        //masterResult.setSentBytes(respond.getResponse().getBytes().length);
    ...
    private String getPlaylistPath(DataRequest respond, Parser parser) throws MalformedURLException {
        ...
        playlistUri = parser.extractUriMaster(respond.getResponse(), this.getRESDATA(), this.getNetwordData(), this.getBandwidthType(), this.getResolutionType(), this.getUrlVideoType());
    ...
    private DataRequest getPlayList(SampleResult playListResult, Parser parser) throws IOException {
        ...
        DataRequest subRespond = parser.getBaseUrl(playlist, playListResult, true);
        ...
        int headerBytes =
                playListResult.getResponseHeaders().length()   // condensed length (without \r)
    ...
    public SampleResult sample(Entry e) {
        ...
        Parser parser = new Parser();//getURLData());
        boolean out = false;
    ...
    public List<SampleResult> getFragments(Parser parser, List<DataFragment> uris, String url) {
        ...
        if (!uris.isEmpty()) {
            ...
            try {
                ...
                DataRequest respond = parser.getBaseUrl(uriString, result, false);
