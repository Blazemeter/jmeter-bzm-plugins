...
public class ViewResultsFullVisualizer extends AbstractVisualizer
	...
	private static final String iconSize = JMeterUtils.getPropDefault(JMeter.TREE_ICON_SIZE,
			JMeter.DEFAULT_TREE_ICON_SIZE);

	private static final ImageIcon imageSuccess = JMeterUtils
			.getImage(JMeterUtils.getPropDefault("viewResultsTree.success", //$NON-NLS-1$
					"vrt/" + iconSize + "/security-high-2.png")); //$NON-NLS-1$ $NON-NLS-2$

	private static final ImageIcon imageFailure = JMeterUtils
			.getImage(JMeterUtils.getPropDefault("viewResultsTree.failure", //$NON-NLS-1$
					"vrt/" + iconSize + "/security-low-2.png")); //$NON-NLS-1$ $NON-NLS-2$

	// Maximum size that we will display
	// Default limited to 10 megabytes
	private static final int MAX_DISPLAY_SIZE = JMeterUtils.getPropDefault("view.results.tree.max_size", 10485760); // $NON-NLS-1$

	// default display order
	private static final String VIEWERS_ORDER = JMeterUtils.getPropDefault("view.results.tree.renderers_order", ""); // $NON-NLS-1$ //$NON-NLS-2$
	...
	private synchronized void updateGui(SampleResult res) {
		...
		if (autoScrollCB.isSelected() && root.getChildCount() > 1) {
			jTree.scrollPathToVisible(
					new TreePath(new Object[] { root, treeModel.getChild(root, root.getChildCount() - 1) }));
		}
	}
	...
	public void addSubResults(DefaultMutableTreeNode currNode, SampleResult res) {
		...
		if (res instanceof HTTP2SampleResult ){
			HTTP2SampleResult resHTTP2 = (HTTP2SampleResult) res;

			for (HTTP2SampleResult child : resHTTP2.getPendingResults()) {
				int id=child.getId();
		...
		if (isTextDataType(res)) {
			...
			if (MAX_DISPLAY_SIZE > 0 && len > MAX_DISPLAY_SIZE) {
				...
				builder.append(JMeterUtils.getResString("view_results_response_too_large_message")) //$NON-NLS-1$
						.append(len).append(" > Max: ").append(MAX_DISPLAY_SIZE).append(", ")
						.append(JMeterUtils.getResString("view_results_response_partial_message")) // $NON-NLS-1$
			...
			if (userObject instanceof SampleResult) {
				if (userObject instanceof HTTP2SampleResult) {
					...
					if (http2Result.isPendingResponse()) {
						/*
						 * set to a different color the request sent that not
						 * get a complete response yet They will be changed to
						 * the corresponding color when response were received
						 */
